...
integrations and adoption. This repository is me learning OpenTelemetry. I will do it by way of an example stack:
...
4. Build the program distribution
    * ```shell
      ./gradlew installDist
      ```
    * The distribution is in `build/install/open-telemetry-playground/`. Notice the "start script" file at `bin/open-telemetry-playground`.
...
5. Run the program with the agent
    * ```shell
      JAVA_OPTS="-javaagent:$(pwd)/opentelemetry-javaagent.jar -Dotel.javaagent.configuration-file=$(pwd)/open-telemetry.properties" ./build/install/open-telemetry-playground/bin/open-telemetry-playground
      ```
...
7. Inspect the metrics in InfluxDB directly
    ...
    * ```shell
      docker exec -it open-telemetry-playground-influxdb-1 influx -precision rfc3339
      ```
    ...
    * ```text
      $ docker exec -it open-telemetry-playground-influxdb-1 influx
